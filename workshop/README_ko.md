...
1. 애플리케이션은 Sqlite3과 구식 Python 문법을 사용하므로 이를 최신 문법으로 업데이트할 수 있습니다.
...
> [!NOTE]
> 왜 안 될 수 있나요? 레거시 Python은 더 이상 지원되지 않는 `distribute`를 사용하므로 의존성 설치가 실패할 수 있습니다.
...
- 기능 테스트를 활용해 집중할 기능의 _슬라이스_를 정하세요.
...
- 오류 출력을 Copilot에게 질문할 때 포함하세요. `#terminalSelection`을 활용하세요.
- Copilot의 답변을 검토하고 추가 질문을 하세요.
- 아직 코드를 수정하지 마세요.

> [!NOTE]
> 오류가 바로 이해되지 않거나 Python 버전이 구버전임을 암시할 수 있습니다. 추가 질문을 통해 오류의 원인을 파악하고 전략을 세우세요. 프로젝트가 구버전 Python으로 작성되었음을 Copilot에 알려줄 수도 있습니다.

### 6. try/except 오류 수정하기

기능 테스트를 활용해 집중할 기능의 _슬라이스_를 정하세요. 이번 단계에서는 try/except 오류를 수정합니다.

- Copilot에게 구버전 Python임을 언급하며 문제 해결을 요청하세요.
...
Pytest는 Python에서 가장 강력한 테스트 프레임워크입니다. 레거시 코드는 `unittest`를 사용하므로, 모든 테스트를 `pytest`로 포팅하세요.
...
- `pyproject.toml`로 프로젝트를 설치하세요. `pip install .` 명령어로 편집 모드 설치가 가능합니다.
...
레거시 프로젝트에는 자동화가 없습니다. GitHub Actions로 테스트와 검증을 자동화하세요. 모든 Pull Request마다 워크플로우가 실행되어 테스트와 설치를 시도해야 합니다.
...
> [!NOTE]
> 자동화는 현대 소프트웨어 개발의 핵심이지만, 완벽하게 만들기까지 시간이 걸릴 수 있습니다. 인내심을 갖고 Copilot의 도움을 받아 오류를 해결해 나가세요.
